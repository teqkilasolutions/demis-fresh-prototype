...
function initStoreLocatorMap() {
  ...
  // Add Special Haridwar Origin Marker featuring Demi's Logo
  const logoPath = (typeof window.themeUri !== 'undefined') ? window.themeUri + '/assets/logo.png' : 'assets/logo.png';
  const haridwarIcon = L.divIcon({
    className: 'haridwar-origin-pin-wrapper',
    html: `
      <div class="haridwar-origin-pin" title="Demi's Started in Haridwar">
        <img src="${logoPath}" alt="Demi's Logo">
  ...
  function getBezierPoints(start, end, numPoints = 35) {
    const points = [];
  ...
  function drawAllSupplyLines() {
    ...
    Object.keys(stateData).forEach(key => {
      ...
      const points = getBezierPoints(start, end, 35);
  ...
  // Helper to create directional arrow marker pointing from Haridwar to destination
  function createMovingArrowMarker(isGreen = false) {
    const iconClass = isGreen ? 'directional-arrow-icon green-arrow' : 'directional-arrow-icon';
    const arrowIcon = L.divIcon({
      className: 'directional-arrow-pin',
      html: `<div class="${iconClass}"><i class="fas fa-location-arrow"></i></div>`,
      iconSize: [22, 22],
      iconAnchor: [11, 11]
    });
    return L.marker(haridwarCoords, { icon: arrowIcon }).addTo(map);
  }

  // Animate a single manual pulse from Haridwar to target state with rotated directional arrow
  function animateManualPulse(key) {
    const start = haridwarCoords;
    const end = stateData[key].coords;
    const points = getBezierPoints(start, end, 35);
    
    const movingMarker = createMovingArrowMarker(true);
    activeArcMarkers.push(movingMarker);

    let step = 0;
    function animate() {
      if (step < points.length) {
        const curPt = points[step];
        movingMarker.setLatLng(curPt);

        if (step < points.length - 1) {
          const nextPt = points[step + 1];
          const px1 = map.latLngToContainerPoint(curPt);
          const px2 = map.latLngToContainerPoint(nextPt);
          const angle = Math.atan2(px2.y - px1.y, px2.x - px1.x) * (180 / Math.PI) - 45;
          const el = movingMarker.getElement();
          if (el) {
            const iconEl = el.querySelector('.directional-arrow-icon');
  ...
  // Send waves of directional arrow pulses from Haridwar to all states simultaneously
  function sendSupplyPulses() {
    ...
    Object.keys(stateData).forEach(key => {
      ...
      const movingMarker = createMovingArrowMarker(false);
      activeArcMarkers.push(movingMarker);
...
/* ── Card 3-Photo Sequential Auto-Rotation on Hover ── */
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.product-card').forEach(card => {
    let rotInterval = null;
    
    card.addEventListener('mouseenter', () => {
      const imgs = card.querySelectorAll('.card-rot-img');
      if (imgs.length <= 1) return;
      
      let currIdx = 1; // Switch to Photo 2 immediately on hover
      imgs.forEach(i => i.classList.remove('active'));
      imgs[currIdx].classList.add('active');
    ...
    card.addEventListener('mouseleave', () => {
      ...
      if (imgs.length > 0) {
        imgs.forEach(i => i.classList.remove('active'));
        imgs[0].classList.add('active'); // Reset to Photo 1 (clean floating PNG)
      }
    });
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getCardStep() {
    const card = carousel.querySelector('.product-card:not([style*="display: none"])');
...
/* Reset Carousel Scroll on Category Tab Switch */
document.addEventListener('DOMContentLoaded', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getVisibleCards() {
    return Array.from(carousel.querySelectorAll('.product-card')).filter(card => {
      const style = window.getComputedStyle(card);
      return style.display !== 'none' && style.visibility !== 'hidden';
    });
  }

  function updateArrowStates(visibleCards) {
    if (!prevBtn || !nextBtn) return;
  ...
  function scrollToCard(index) {
    ...
    currentCardIndex = Math.max(0, Math.min(visibleCards.length - 1, index));
    const targetCard = visibleCards[currentCardIndex];
    
    if (targetCard) {
      targetCard.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }
    updateArrowStates(visibleCards);
  }
  ...
  // Reset index when category tabs change
  document.querySelectorAll('.category-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      setTimeout(() => {
        carousel.scrollLeft = 0;
  ...
  // Track manual touch/wheel scroll index sync
...
/* ── Guaranteed Card-by-Card Carousel Arrow Navigation ── */
document.addEventListener('DOMContentLoaded', () => {
  const carouselWrapper = document.querySelector('.products-carousel-wrapper');
  if (!carouselWrapper) return;
  ...
  function scrollToCard(index) {
    ...
    if (targetCard) {
      const carouselCenterOffset = (carousel.clientWidth - targetCard.offsetWidth) / 2;
      const targetScrollLeft = targetCard.offsetLeft - carouselCenterOffset;
      carousel.scrollTo({
  ...
  // Sync index on touch/drag scroll
  let scrollTimer = null;
  carousel.addEventListener('scroll', () => {
    clearTimeout(scrollTimer);
    scrollTimer = setTimeout(() => {
